...
export class RolesManager extends cacheManager<Snowflake, GuildRoles>{
    ...
    async fetchall(guildId: Snowflake){
        ...
        if(roles && Array.isArray(roles)) roles.forEach((role)=> {
    ...
    searchlist(ids: string[]){
        return this.cache.filter((role)=> { if( ids.includes(role.id)) return role; return undefined }).filter((x)=> x != undefined);
    };
